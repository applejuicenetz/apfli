...
#include "../include/md5_loc.h"

//static char *rcs_id = "$Id: md5.c,v 1.5 2000/03/09 04:06:41 gray Exp $";
 ...
 */
static void
process_block(md5_t * md5_p, const void *buffer,
 ...
 */
void
md5_process(md5_t * md5_p, const void *buffer, const unsigned int buf_len)
{
	...
	if (in_block > MD5_BLOCK_SIZE) {
	    process_block(md5_p, md5_p->md_buffer,
 ...
 * signature - A 16 byte buffer that will contain the MD5 signature.
 */
void md5_buffer(char *buffer, unsigned int buf_len, void *signature)
{
    md5_t md5;
